...
//  Maze
//
//  Created by Arisa Muto on 2019/04/26.
...
class ViewController: UIViewController {
    ...
    //wallViewのプレームの情報を入れておく配列
    var wallRectArray = [CGRect]()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        // Do any additional setup after loading the view, typcially from a nib.
    ...
    func startAccelerometer() {
        ...
        let handler: CMAccelerometerHandler = {(CMAccelerometerData:CMAccelerometerData?, error: Error?) -> Void in
            //let handler: CMAccelerometerHandler = {(CM)
            ...
            //画面上からプレイヤーがはみ出しそうだったら、poseX/poseYを修正
    ...
    func gameCheck(result: String, message : String){
        ...
        let gameCheckAlter: UIAlertController = UIAlertController(title: result, message: message, preferredStyle: .alert)
        ...
        gameCheckAlter.addAction(retryAction)
        
        self.present(gameCheckAlter, animated: true, completion: nil)
    }
    func retry() {
        //プレイヤー位置を初期化
        playerView.center = startView.center
